...
async def _lifespan(app: "FastAPI"):
    ...
    project_hermes_services = getattr(
        app.state,
        "project_hermes",
        None,
    )
    try:
        if (
            project_hermes_services is not None
            and project_hermes_services.supervisor is not None
        ):
            await project_hermes_services.supervisor.start()
        yield
    finally:
        if (
            project_hermes_services is not None
            ...
            await project_hermes_services.supervisor.stop()
        if project_hermes_services is not None:
            project_hermes_services.close()
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Project Hermes Light",         "description": "InferScope-inspired light operations dashboard"},
    {"name": "default-large", "label": "Project Hermes Light (Large)", "description": "Project Hermes Light with bigger fonts and roomier spacing"},
...
def _mount_project_hermes_routes() -> None:
    """Mount ProjectHermes only when its versioned config is present."""

    from project_hermes.web_integration import mount_project_hermes

    services = mount_project_hermes(
        app,
        require_dashboard_token=_require_token,
    )
    if services is not None:
        _log.info(
            "Mounted ProjectHermes API routes: /api/v2/project-hermes/"
        )


_mount_project_hermes_routes()
