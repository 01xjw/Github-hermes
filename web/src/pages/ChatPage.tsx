...
// Light terminal palette matching the supplied InferScope reference. Keep
// ANSI colors saturated enough to remain readable on the white canvas.
const DEFAULT_TERMINAL_BACKGROUND = "#ffffff";
const DEFAULT_TERMINAL_FOREGROUND = "#1e293b";
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        <div
          className={cn(
            "relative flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-lg border border-[#e2e8f0]",
            "p-2 sm:p-3",
          )}
          style={{
        ...
        >
          ...
          {showReconnectOverlay && (
            <div className="absolute inset-x-3 top-3 z-20 flex justify-center sm:inset-x-auto sm:right-3 sm:justify-end">
              <div className="flex max-w-[min(28rem,calc(100vw-3rem))] flex-col items-start gap-2 rounded-md border border-amber-200 bg-white/95 px-3 py-2 text-xs text-amber-700 shadow-lg">
          ...
          {showResumeLoadingOverlay && (
            ...
            >
              <div className="max-w-[min(28rem,calc(100vw-3rem))] rounded-md border border-[#e2e8f0] bg-white/95 px-4 py-3 text-center text-xs tracking-wide text-[#1e293b] shadow-lg">
                {PTY_RESUME_LOADING_MESSAGE}
              </div>
            </div>
          )}
          ...
          {ptyState === "ended" && (
            <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-3 bg-white/90">
              <div className="text-sm tracking-wide text-[#64748b]">
                Session ended.
              </div>
              <Button
