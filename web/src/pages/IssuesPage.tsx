    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <h3 className="mt-1.5 line-clamp-2 text-[13px] font-semibold leading-5">
            {candidate.title}
          </h3>
        </div>
        <a
...
function IssueDetail({
  ...
  if (loading && !detail) {
    return <div className="flex min-h-[28rem] items-center justify-center"><Spinner /></div>;
  }
  if (!detail) {
    return (
    ...
    <div className="divide-y divide-border">
      <section className="px-5 py-4">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          {work ? <WorkStatusBadge status={work.status} /> : <FeedStatus item={{ candidate, work_item: null, screening: detail.screening }} />}
        ...
        {detail.screening ? (
          <div className="mt-3 space-y-3">
            ...
            <dl className="grid gap-2 text-[10px] sm:grid-cols-2">
              <div><dt className="text-muted-foreground">CPU / memory</dt><dd className="project-data mt-1">{detail.screening.required_environment.minimum_cpu_cores} cores · {detail.screening.required_environment.minimum_memory_gib} GiB</dd></div>
              <div><dt className="text-muted-foreground">GPU</dt><dd className="project-data mt-1">{detail.screening.required_environment.gpu_count} · {detail.screening.required_environment.gpu_architectures.join(", ") || "none"}</dd></div>
              <div><dt className="text-muted-foreground">OS / CPU arch</dt><dd className="project-data mt-1">{detail.screening.required_environment.operating_systems.join(", ") || "any"} · {detail.screening.required_environment.cpu_architectures.join(", ") || "any"}</dd></div>
              <div><dt className="text-muted-foreground">External requirements</dt><dd className="project-data mt-1">{detail.screening.required_environment.network_access_required ? "network" : "no network"} · {detail.screening.required_environment.external_system_write_required ? "external write" : "local only"}</dd></div>
              <div className="sm:col-span-2"><dt className="text-muted-foreground">Unresolved dependencies</dt><dd className="project-data mt-1">{detail.screening.required_environment.external_dependencies.join(", ") || "none"}</dd></div>
            </dl>
            {detail.screening.evidence.length ? (
              <ul className="list-disc space-y-1 pl-4 text-xs leading-5 text-muted-foreground">
                {detail.screening.evidence.map((value) => <li key={value}>{value}</li>)}
              </ul>
            ) : null}
            ...
            <p className="project-id text-[9px] text-muted-foreground">
              profile {detail.screening.profile_digest.slice(0, 16)} · {detail.screening.model_provider}/{detail.screening.model} · {formatProjectTime(detail.screening.screened_at)}
            </p>
            {detail.screening.screening_trigger === "operator_rescreen" ? (
              <div className="rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-[10px] leading-4 text-blue-800">
                <p>
                  Re-screen requested by {detail.screening.requested_by ?? "operator"}: {detail.screening.request_reason}
                </p>
                {detail.screening.probe_evidence.length ? (
                  <ul className="mt-1 list-disc space-y-1 pl-4">
                    {detail.screening.probe_evidence.map((value) => <li key={value}>{value}</li>)}
                  </ul>
                ) : null}
              </div>
            ) : null}
            {detail.screening_history.length ? (
              <details className="rounded-md border border-border bg-muted/20 px-3 py-2">
                <summary className="cursor-pointer text-[10px] font-semibold text-muted-foreground">
                  Screening history · {detail.screening_history.length} immutable revision{detail.screening_history.length === 1 ? "" : "s"}
                </summary>
                <ol className="mt-3 space-y-3">
                  {detail.screening_history.slice().reverse().map((revision) => (
                    <li key={`${revision.candidate_snapshot_digest}-${revision.revision}`} className="border-l-2 border-blue-200 pl-3 text-[10px] leading-4">
                      <div className="flex flex-wrap items-center gap-2">
                      ...
                      {revision.request_reason ? <p className="mt-1 text-muted-foreground">Request: {revision.request_reason}</p> : null}
                    </li>
                  ))}
                </ol>
              </details>
            ...
            {!work ? (
              <div className="rounded-md border border-border bg-card px-3 py-3">
                ...
                <p className="mt-1 text-[10px] leading-4 text-muted-foreground">
                  Supply the audit reason and bounded evidence produced by a real probe. This creates a new immutable screening revision; it does not approve the Issue for Work.
                </p>
          ...
          {detail.operator_selected ? (
            ...
            <div className="mt-3 flex flex-col items-start gap-2">
              <p className="text-xs leading-5 text-muted-foreground">
                The screening Subagent marked this Issue SELECT, but scanning still cannot start Work. Confirm it here after reviewing the reason and machine requirements.
              </p>
        ...
        <div>
          ...
          {candidate.eligible ? (
            ...
            <ul className="mt-2 list-disc space-y-1 pl-4 text-xs leading-5 text-muted-foreground">
              {candidate.filter_reasons.map((reason) => <li key={reason}>{reason}</li>)}
            </ul>
          )}
        </div>
        <div>
      ...
      {work?.environment_status === "verified" && work.resource_requirements ? (
        <section className="px-5 py-4">
          ...
          <dl className="mt-3 grid grid-cols-2 gap-3 text-[10px] sm:grid-cols-4">
            <div><dt className="text-muted-foreground">CPU</dt><dd className="project-data mt-1 font-semibold">{work.resource_requirements.cpu_request} / {work.resource_requirements.cpu_limit}</dd></div>
            <div><dt className="text-muted-foreground">Memory</dt><dd className="project-data mt-1 font-semibold">{work.resource_requirements.memory_request} / {work.resource_requirements.memory_limit}</dd></div>
            <div><dt className="text-muted-foreground">GPU</dt><dd className="project-data mt-1 font-semibold">{work.resource_requirements.gpu_count} · {work.resource_requirements.gpu_architecture ?? "none"}</dd></div>
            <div><dt className="text-muted-foreground">Worker model</dt><dd className="project-data mt-1 break-all font-semibold">{work.resource_requirements.worker_model}</dd></div>
          </dl>
      ...
      {events.length ? (
        <section className="px-5 py-4">
          <h3 className="text-[10px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">Recent Work events</h3>
          <ol className="mt-3 space-y-2">
            {events.slice(-8).reverse().map((event) => (
              <li key={event.event_id} className="flex items-center justify-between gap-4 text-[10px]">
                ...
                <time className="project-data shrink-0 text-[9px] text-muted-foreground">{formatProjectTime(event.created_at)}</time>
              </li>
            ))}
          </ol>
        </section>
...
export default function IssuesPage() {
  ...
  const refreshFeed = useCallback(async (background = false) => {
    ...
    try {
      const [nextFeed, nextOverview] = await Promise.all([
        api.listProjectIssues({
          ...
          eligible:
            eligibility === "all" ? undefined : eligibility === "matched",
          search: deferredSearch,
          status: status === "all" ? undefined : [status],
          screeningDecision:
            screening === "all" ? undefined : [screening],
          limit: PAGE_SIZE,
  ...
  useEffect(() => {
    ...
    api.getProjectIssue(selectedId)
      .then((next) => {
  ...
  const activeRescreenReason = rescreenDraft.candidateId === selectedId
    ? rescreenDraft.reason
    : "";
  const activeProbeEvidence = rescreenDraft.candidateId === selectedId
    ? rescreenDraft.probeEvidence
    : "";
    ...
    <div className="project-hermes-ui space-y-4 py-4">
      <ProjectPanel bodyClassName="px-4 py-3">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-blue-600">Live Issue intelligence</p>
            ...
            {overview?.operator_selection_required ? (
              <p className="mt-2 text-[10px] font-medium text-orange-700">
                Guided training mode: the generic Subagent screens every snapshot; only your explicit confirmation starts the normal workflow.
              </p>
            ) : null}
          </div>
        ...
        <div className="mt-3 flex flex-col gap-2 border-t border-border pt-3 lg:flex-row lg:items-center">
          ...
          <div className="flex flex-wrap gap-1">
            {(["SELECT", "DEFER", "REJECT", "PENDING", "all"] as ScreeningFilter[]).map((value) => (
              <button key={value} type="button" onClick={() => setScreening(value)} className={cn("rounded-full border px-2.5 py-1 text-[9px] font-semibold uppercase transition", screening === value ? "border-green-600 bg-green-600 text-white" : "border-border bg-card text-muted-foreground hover:border-green-600 hover:text-green-700")}>{value}</button>
            ))}
          </div>
          <div className="flex flex-wrap gap-1">
            {(["matched", "filtered", "all"] as EligibilityFilter[]).map((value) => (
              <button key={value} type="button" onClick={() => setEligibility(value)} className={cn("rounded-full border px-2.5 py-1 text-[9px] font-semibold uppercase transition", eligibility === value ? "border-blue-500 bg-blue-500 text-white" : "border-border bg-card text-muted-foreground hover:border-blue-500 hover:text-blue-600")}>{value}</button>
            ))}
          </div>
          <select value={status} onChange={(event) => setStatus(event.target.value as StatusFilter)} className="project-data rounded-md border border-border bg-card px-2.5 py-1.5 text-[10px] outline-none focus:border-blue-500">
            {STATUS_FILTERS.map((value) => <option key={value} value={value}>status: {value}</option>)}
          </select>
        </div>
      </ProjectPanel>
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(21rem,0.9fr)_minmax(28rem,1.1fr)]">
        <ProjectPanel
          ...
          subtitle={selectedItem ? `${selectedItem.candidate.repository} #${selectedItem.candidate.issue_number} selected` : "Waiting for a matching Issue"}
        ...
        >
          <div className="max-h-[calc(100dvh-20rem)] min-h-[30rem] overflow-y-auto">
            {loading && !feed ? (
              <div className="flex min-h-[30rem] items-center justify-center"><Spinner /></div>
            ) : feed?.issues.length ? (
              feed.issues.map((item) => (
                <IssueFeedRow key={item.candidate.candidate_id} item={item} selected={item.candidate.candidate_id === selectedId} onSelect={() => setSelectedId(item.candidate.candidate_id)} />
              ))
            ) : (
          ...
          <footer className="flex items-center justify-between border-t border-border px-4 py-2.5">
            <Button ghost size="icon" type="button" disabled={page === 0} onClick={() => setPage((value) => Math.max(0, value - 1))}><ChevronLeft /></Button>
            <span className="project-data text-[9px] text-muted-foreground">{page + 1} / {totalPages}</span>
            <Button ghost size="icon" type="button" disabled={page + 1 >= totalPages} onClick={() => setPage((value) => value + 1)}><ChevronRight /></Button>
          </footer>
        </ProjectPanel>
        ...
        <ProjectPanel title="Issue detail" subtitle="Evidence, Main Hermes plan, and verified resources">
          <div className="max-h-[calc(100dvh-15rem)] overflow-y-auto">
            <IssueDetail
              ...
              onRescreenReasonChange={(value) => setRescreenDraft((current) => ({
                candidateId: selectedId,
                reason: value,
                probeEvidence: current.candidateId === selectedId
                  ? current.probeEvidence
                  : "",
              }))}
              onProbeEvidenceChange={(value) => setRescreenDraft((current) => ({
                candidateId: selectedId,
