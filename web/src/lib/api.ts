...
export const api = {
  ...
  getGitStatus: (path: string) =>
    fetchJSON<GitRepositoryStatus | null>(
  ...
  getGitReviewList: (
    ...
    return fetchJSON<GitReviewListResponse>(
      `/api/git/review/list?${query.toString()}`,
    );
  },
  getGitReviewDiff: (
    path: string,
    ...
    if (base) query.set("base", base);
    return fetchJSON<GitReviewDiffResponse>(
      `/api/git/review/diff?${query.toString()}`,
    );
  },
  getGitShipInfo: (path: string) =>
    fetchJSON<GitShipInfoResponse>(
      `/api/git/review/ship-info?path=${encodeURIComponent(path)}`,
    ),
  getPollingOverview: (repositoryId?: number) => {
    const query = repositoryId ? `?repository_id=${repositoryId}` : "";
    return fetchJSON<PollingOverviewResponse>(
  ...
  listPollingRuns: (limit = 50, offset = 0) =>
    fetchJSON<PollingRunListResponse>(
      `/api/v2/project-hermes/polling/runs?limit=${limit}&offset=${offset}`,
    ),
  listPollingRepositories: () =>
    fetchJSON<PollingRepositoryListResponse>(
      "/api/v2/project-hermes/polling/repositories",
    ),
  listProjectIssues: (params: ProjectIssueQuery = {}) => {
    const query = new URLSearchParams();
    if (params.repositoryId) {
      query.set("repository_id", String(params.repositoryId));
    }
    if (params.eligible !== undefined) {
      query.set("eligible", String(params.eligible));
    }
    if (params.search?.trim()) query.set("search", params.search.trim());
    if (params.runId) query.set("run_id", params.runId);
    for (const status of params.status ?? []) query.append("status", status);
    for (const decision of params.screeningDecision ?? []) {
      query.append("screening_decision", decision);
    }
    query.set("limit", String(params.limit ?? 100));
    query.set("offset", String(params.offset ?? 0));
    return fetchJSON<ProjectIssueListResponse>(
      `/api/v2/project-hermes/issues?${query.toString()}`,
    );
  },
  getProjectIssue: (candidateId: string) =>
    fetchJSON<ProjectIssueDetailResponse>(
      `/api/v2/project-hermes/issues/${encodeURIComponent(candidateId)}`,
    ),
  selectProjectIssue: (candidateId: string) =>
    fetchJSON<ProjectIssueDetailResponse>(
      `/api/v2/project-hermes/issues/${encodeURIComponent(candidateId)}/select`,
      { method: "POST" },
    ),
  rescreenProjectIssue: (
    candidateId: string,
    payload: { reason: string; probe_evidence: string[] },
  ) =>
    fetchJSON<ProjectIssueDetailResponse>(
      `/api/v2/project-hermes/issues/${encodeURIComponent(candidateId)}/rescreen`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  listProjectWorkItems: (params: ProjectWorkQuery = {}) => {
    const query = new URLSearchParams();
    if (params.repositoryId) {
      query.set("repository_id", String(params.repositoryId));
    }
    ...
    query.set("limit", String(params.limit ?? 100));
    query.set("offset", String(params.offset ?? 0));
    return fetchJSON<ProjectWorkListResponse>(
      `/api/v2/project-hermes/work-items?${query.toString()}`,
    );
  },
  getProjectWorkItem: (workItemId: string) =>
    fetchJSON<ProjectWorkDetailResponse>(
      `/api/v2/project-hermes/work-items/${encodeURIComponent(workItemId)}`,
    ),
  listInternalPullRequestCandidates: (limit = 100, offset = 0) =>
    fetchJSON<InternalPullRequestCandidateListResponse>(
      `/api/v2/project-hermes/pull-request-candidates?limit=${limit}&offset=${offset}`,
    ),
  getInternalPullRequestCandidate: (candidateId: string) =>
    fetchJSON<InternalPullRequestCandidate>(
      `/api/v2/project-hermes/pull-request-candidates/${encodeURIComponent(candidateId)}`,
    ),
...
export interface GitReviewDiffResponse {
  diff: string;
}
...
export interface GitShipInfoResponse {
  ghReady: boolean;
  pr: GitPullRequestSummary | null;
}

export type InternalPullRequestLockState =
  | "draft"
  | "approval_pending"
  | "immutable_approved";

export interface InternalPullRequestCandidateSummary {
  candidate_id: string;
  task_id: string;
  title: string;
  repository: string;
  ...
  head_ref: string;
  head_sha: string;
  lock_state: InternalPullRequestLockState;
  file_count: number;
  commit_count: number;
...
export interface InternalPullRequestCandidateListResponse {
...
// ── Scheduled Issue polling + Main Hermes Work projections ───────────
...
export interface PollingRepository {
  ...
  last_status: RepositoryScanStatus | null;
  last_error: string | null;
  created_at: string;
  updated_at: string;
}
...
export type ProjectScreeningFilter = ProjectScreeningDecision | "PENDING";
export type ProjectMachineCompatibility =
  | "COMPATIBLE"
  | "NEEDS_PROBE"
  | "INCOMPATIBLE";
export type ProjectScreeningTaskKind =
  | "LOCAL_CODE_OR_TEST"
  | "LOCAL_REPRODUCTION"
  | "EXTERNAL_OPERATION"
  | "NON_ACTIONABLE_REPORT";

export interface ProjectScreeningCounts {
  SELECT: number;
  DEFER: number;
  REJECT: number;
  PENDING: number;
...
export interface PollingSupervisorStatus {
  running: boolean;
  stopping: boolean;
  polling_runs: number;
  manager_turns: number;
  reconcile_count: number;
  reviewer_turns: number;
  screening_turns: number;
  last_polled_at: string | null;
  last_manager_turn_at: string | null;
...
export interface PollingOverviewResponse {
  task: PollingTask;
  latest_run: PollingRun | null;
  selected_repository_id: number | null;
  repository_count: number;
  repositories_in_view: number;
  issues_seen: number;
  polling_run_count: number;
  candidate_counts: { total: number; matched: number; filtered: number };
  screening_counts: ProjectScreeningCounts;
  work_counts: Record<ProjectWorkStatus, number>;
  processed: { today: number; this_week: number };
  repository_stats: PollingRepositoryStat[];
  manager: {
    session_id: string;
...
export interface PollingRepositoryListResponse {
  repositories: PollingRepository[];
  total: number;
}

export interface ProjectIssueCandidate {
  candidate_id: string;
  repository_id: number;
  repository: string;
  issue_number: number;
...
export interface ProjectScreeningEnvironment {
  ...
  external_system_write_required: boolean;
  external_dependencies: string[];
}

export interface ProjectIssueScreening {
  candidate_id: string;
  candidate_snapshot_digest: string;
  decision: ProjectScreeningDecision;
  machine_compatibility: ProjectMachineCompatibility;
  task_kind: ProjectScreeningTaskKind;
  reason: string;
  required_environment: ProjectScreeningEnvironment;
  evidence: string[];
  ...
  agent_digest: string;
  revision: number;
  screening_trigger: "initial" | "operator_rescreen";
...
export interface ProjectWorkResources {
  cpu_request: string;
  cpu_limit: string;
...
export interface ProjectWorkItem {
  ...
  environment_status: WorkEnvironmentStatus;
  environment_verified_at: string | null;
  named_baseline: string | null;
  resource_requirements: ProjectWorkResources | null;
  task_id: string | null;
  run_id: string | null;
...
export interface ProjectIssueFeedItem {
  ...
  work_item: ProjectWorkItem | null;
  screening: ProjectIssueScreening | null;
}
...
export interface ProjectIssueQuery {
  ...
  search?: string;
  status?: ProjectWorkStatus[];
  screeningDecision?: ProjectScreeningFilter[];
  limit?: number;
  offset?: number;
}

export interface ProjectIssueListResponse {
  issues: ProjectIssueFeedItem[];
  total: number;
  limit: number;
  offset: number;
}
...
export interface ProjectIssueDetailResponse {
  ...
  work_item: ProjectWorkItem | null;
  screening: ProjectIssueScreening | null;
  screening_history: ProjectIssueScreening[];
...
export interface ProjectWorkListResponse {
  work_items: ProjectWorkItem[];
  counts: Record<ProjectWorkStatus, number>;
  total: number;
  limit: number;
  offset: number;
}

export interface ProjectWorkDetailResponse {
...
export interface InternalPullRequestCommit {
  sha: string;
  message: string;
  author_name: string;
...
export interface InternalPullRequestCandidateFile {
  path: string;
  status: "?" | "A" | "C" | "D" | "M" | "R" | "T" | "U";
  added: number;
  removed: number;
  is_binary: boolean;
...
export interface InternalPullRequestCheck {
  name: string;
  ...
  summary: string;
  evidence_ids: string[];
  validation_overlay_digest: string | null;
  started_at: string | null;
  completed_at: string | null;
}

export interface InternalPullRequestReview {
  review_id: string;
  role: string;
  reviewer: string;
  verdict:
...
export interface InternalCandidateAccountingRound {
  round_id: string;
  kind: string;
  role: string;
  outcome: string;
...
export interface InternalCandidateAccounting {
  wall_clock_duration_ms: number;
  active_duration_ms: number;
  estimated_llm_cost_usd: number;
  cost_complete: boolean;
  rounds: InternalCandidateAccountingRound[];
}

export interface InternalCandidateComparison {
  summary: string;
  defects: string[];
  coverage: string[];
  minimality: string[];
...
export interface InternalPullRequestCandidate {
  ...
  head_ref: string;
  head_sha: string;
  commits: InternalPullRequestCommit[];
  files: InternalPullRequestCandidateFile[];
  checks: InternalPullRequestCheck[];
  reviews: InternalPullRequestReview[];
  accounting: InternalCandidateAccounting;
  lock_state: InternalPullRequestLockState;
  lock_digest: string | null;
  locked_by: string | null;
  locked_at: string | null;
  post_lock_comparison: InternalCandidateComparison | null;
  created_at: string;
  updated_at: string;
}

export interface InternalPullRequestCandidateFileDiffResponse {
  candidate_id: string;
