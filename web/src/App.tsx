...
export default function App() {
  ...
  const builtinNav = useMemo(() => {
    ...
    // Chat sits immediately before Hermes Tools. Anchoring on the path keeps
    // that placement stable as built-in entries are added above it.
    const anchor = BUILTIN_NAV_REST.findIndex((item) => item.path === "/legacy");
    const at = anchor >= 0 ? anchor : BUILTIN_NAV_REST.length;
    return [
      ...BUILTIN_NAV_REST.slice(0, at),
      CHAT_NAV_ITEM,
      ...BUILTIN_NAV_REST.slice(at),
    ];
  }, [embeddedChat]);

  const sidebarNav = useMemo(() => {
    const partitioned = partitionSidebarNav(builtinNav, manifests);
    return { ...partitioned, pluginItems: [] as NavItem[] };
  ...
  return (
    <ProfileProvider>
    <div
      data-project-hermes-shell
      data-layout-variant={layoutVariant}
      className="project-hermes-shell flex h-dvh max-h-dvh min-h-0 flex-col overflow-hidden bg-[#f8f9fb] text-[#1e293b] antialiased"
    >
      <SelectionSwitcher />
      ...
      >
        ...
        <Typography className="font-mono-ui text-[0.95rem] font-bold leading-none text-[#1e293b]">
          Hermes Agent
        </Typography>
      </header>
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden pt-14 lg:pt-0">
        <div className="flex min-h-0 min-w-0 flex-1">
          <aside
            ...
            className={cn(
              "project-hermes-sidebar fixed top-0 left-0 z-50 flex h-dvh max-h-dvh w-[200px] min-h-0 flex-col bg-white font-sans text-[#1e293b]",
            ...
            style={{ clipPath: "var(--component-sidebar-clip-path)" }}
          >
            <div
              className={cn(
            ...
            >
              ...
              >
                ...
                <div className="min-w-0">
                  <Typography className="font-mono-ui truncate text-[15px] font-bold leading-none text-[#1e293b]">
                    Hermes Agent
                  </Typography>
                  <p className="mt-1 truncate text-[9px] font-medium uppercase tracking-[0.1em] text-[#64748b]">
                    Issue operations
                  </p>
                </div>
              </div>
          ...
          <PageHeaderProvider pluginTabs={pluginTabMeta}>
            <div
              className={cn(
                "relative z-2 flex min-w-0 min-h-0 flex-1 flex-col bg-[#f8f9fb]",
                "px-3 sm:px-6",
                isChatRoute
      ...
      <NavLink
        ...
        className={({ isActive }) =>
          cn(
            ...
            isActive
              ? "bg-[#eff6ff] font-semibold text-[#3b82f6]"
      ...
      >
        {({ isActive }) => (
          <>
            ...
            <span
              aria-hidden
              className="pointer-events-none absolute inset-y-0.5 left-1.5 right-1.5 bg-[#3b82f6] opacity-0 transition-opacity duration-200 group-hover/nav:opacity-[0.03]"
            />

            {isActive && (
              <span
                aria-hidden
                className="absolute bottom-0 left-0 top-0 w-[3px] bg-[#3b82f6]"
              />
            )}
          </>
        )}
