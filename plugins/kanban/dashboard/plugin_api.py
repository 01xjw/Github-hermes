...
@lru_cache(maxsize=1)
def _project_hermes_stores() -> tuple[Any, Any, Any] | None:
    """Open the configured ProjectHermes stores for task-detail joins."""

    config_path = os.environ.get("PROJECT_HERMES_CONFIG", "").strip()
    if not config_path:
        return None
    path = Path(config_path).expanduser().resolve()
    if not path.is_file():
        log.warning("ProjectHermes config is unavailable: %s", path)
        return None
    try:
        from project_hermes.accounting import open_accounting_store
        from project_hermes.assurance_store import open_assurance_store
        from project_hermes.config import load_config as load_project_config
        from project_hermes.store import open_run_store

        config = load_project_config(path)
        return (
            open_run_store(config.control_plane),
            open_accounting_store(config.control_plane),
            open_assurance_store(config.control_plane),
        )
    except Exception as exc:
        log.warning("ProjectHermes accounting stores failed to open: %s", exc)
        return None


def _project_hermes_accounting(task_id: str) -> dict[str, Any] | None:
    stores = _project_hermes_stores()
    if stores is None:
        return None
    run_store, accounting_store, assurance_store = stores
    try:
        from project_hermes.api import task_accounting_payload

        return task_accounting_payload(
            task_id,
            store=run_store,
            accounting=accounting_store,
            assurance=assurance_store,
        )
    except KeyError:
        return None
    except Exception as exc:
        log.warning(
            "ProjectHermes accounting lookup failed for %s: %s",
            task_id,
            exc,
        )
        return None
...
def get_task(
    ...
    try:
        ...
        accounting = _project_hermes_accounting(task_id)
        if accounting is not None:
            task_d["accounting"] = accounting
        return {
            "task": task_d,
