...
(function () {
  ...
  function fmtAccountingDuration(milliseconds) {
  ...
  function fmtAccountingCost(amount, status) {
    if (status === "not_applicable") return "—";
    if (amount === null || amount === undefined || status === "unknown") return "unpriced";
    const numeric = Number(amount);
    if (!Number.isFinite(numeric)) return "unpriced";
  ...
  function AccountingSection(props) {
    ...
    if (!accounting || !accounting.summary) return null;
    const summary = accounting.summary;
    const rounds = summary.rounds || [];
    const reviews = accounting.review_cycles || [];
    const reviewsByCycle = {};
    reviews.forEach(function (review) {
      if (review.review_cycle !== null && review.review_cycle !== undefined) {
        reviewsByCycle[String(review.review_cycle)] = review;
      }
    });
    ...
    return h("div", { className: "hermes-kanban-section" },
      h("div", { className: "hermes-kanban-section-head" },
        `${tx(t, "taskAccounting", "Time & LLM cost")} (${rounds.length})`),
      rounds.map(function (round, index) {
        const review = round.review_cycle
          ? reviewsByCycle[String(round.review_cycle)]
          : null;
        const modelLabel = round.model || round.runtime_name || round.kind;
        const costLabel = fmtAccountingCost(
          round.estimated_cost_usd,
          round.cost_status,
        );
        return h("details", {
          key: round.record_id || index,
          className: "hermes-kanban-run",
        },
          h("summary", { className: "hermes-kanban-run-head" },
            h("span", { className: "hermes-kanban-run-outcome" },
              round.outcome || round.kind),
            h("span", { className: "hermes-kanban-run-profile" },
              `${round.role} · ${modelLabel}`),
            h("span", { className: "hermes-kanban-run-elapsed" },
              fmtAccountingDuration(round.duration_ms)),
            h("span", { className: "hermes-kanban-run-ago" }, costLabel),
          ),
          h("div", { className: "hermes-kanban-run-summary" },
            `${Number(round.input_tokens || 0).toLocaleString()} input · ` +
            `${Number(round.output_tokens || 0).toLocaleString()} output · ` +
            `${Number(round.reasoning_tokens || 0).toLocaleString()} reasoning · ` +
            `${Number(round.api_calls || 0).toLocaleString()} API calls`,
          ),
          round.model_profile
            ? h("div", { className: "text-xs text-muted-foreground" },
                `Profile: ${round.model_profile}` +
                (round.reasoning_mode ? ` · thinking: ${round.reasoning_mode}` : ""))
            : null,
          review
            ? h("div", { className: "hermes-kanban-comment" },
                h("div", { className: "hermes-kanban-comment-head" },
                  h("span", { className: "hermes-kanban-comment-author" },
                    `Review ${review.review_cycle}: ${review.progress || review.verdict}`)),
                h("div", { className: "text-sm" },
                  review.progress_explanation || review.verdict),
              )
            : null,
        );
      }),
  ...
  function TaskDetail(props) {
    ...
    const accounting = t.accounting || null;
    const accountingSummary = accounting && accounting.summary
      ? accounting.summary
      : null;
    const showAccountingTotals = accountingSummary && (
      t.status === "done" || accountingSummary.completed_at
    );
    ...
    return h("div", { className: "hermes-kanban-drawer-body" },
      ...
      h("div", { className: "hermes-kanban-drawer-meta" },
        ...
        showAccountingTotals ? h(MetaRow, {
          label: tx(i18n, "totalElapsed", "Total elapsed"),
          value: fmtAccountingDuration(accountingSummary.wall_clock_duration_ms),
        }) : null,
        showAccountingTotals ? h(MetaRow, {
          label: tx(i18n, "totalLlmCost", "Total LLM cost"),
          value: fmtAccountingCost(
            accountingSummary.estimated_llm_cost_usd,
            "estimated",
          ) + (accountingSummary.cost_complete
            ? ""
            : ` (${accountingSummary.unknown_cost_rounds} unpriced)`),
        }) : null,
      ),
      h(StatusActions, {
        task: t,
      ...
      h(AccountingSection, { accounting: accounting }),
